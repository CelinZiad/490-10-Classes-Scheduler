...
from fitness import (evaluate_population, fitness_function,
                     display_fitness_details, display_schedule_structure)
...
from room_management import (load_room_assignments, create_room_timetables,
                             display_room_timetable, validate_room_timetables)
...
def initialize_population(courses: List[Course], population_size: int,
    ...
    for i in range(population_size):
        ...
        for c in courses:
            ...
            if not initialize_course_with_validation(
                    course_copy,
                    room_assignments=room_assignments,
                    existing_schedule=individual):
                print(f"Warning: Could not fully initialize "
                      f"{course_copy.subject}{course_copy.catalog_nbr}")
...
def run_one_generation(population: List[List[Course]],
    ...
    offspring_fitness = [
        fitness_function(off, core_sequences=seq.year,
                         room_assignments=room_assignments)
...
if __name__ == "__main__":
    ...
    # --- Extract data from DB into CSV files ---------------------------
    ...
    fitness_scores = []
    for i, individual in enumerate(population):
        score = fitness_function(individual, core_sequences=seq.year,
                                 room_assignments=room_assignments)
    ...
    # --- Export results -------------------------------------------------
    display_export_summary(population[best_idx],
    ...
    try:
        success = export_to_scheduleterm_format(
            ...
            year=ACADEMIC_YEAR,
            season=TARGET_SEASON,
            csv_output_path="scheduleterm_output.csv"
        )
    except Exception:
        pass